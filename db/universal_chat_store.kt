...
class universal_chat_store(val context_ : Context, val factory : SQLiteDatabase.CursorFactory?) :
    ...
    companion object {
        ...
        const val PRIVATE_GROUP =
            "_PRIVATE_GROUP_"   // if this is true then nobody can modify the change except group admin
        ...
        const val GROUP_INDIVIDUAL_PRIVATE =
            "_individual_private_"    // this is only in the local database not in server ,which contains fingerprint private true or false
        ...
        const val DELIVERED_ =
            "_delivered_msg_"     // If this message is your then it will contains HashMap<Int,String>() otherwise ""
    }
...
//      DB.delete("$TABLE_NAME_UNIVERSAL_CHAT","${MESSAGE_NUMBER}='${msg_number}'",null)
        ...
        try {
            ...
            if (cursor.count > 0) {
                ...
                do {
                    ...
                    private_group_chat =
                        cursor.getString(cursor.getColumnIndexOrThrow(GROUP_INDIVIDUAL_PRIVATE))
        ...
        try {
            ...
            if (cursor.count > 0) {
                ...
                do {
                    ...
                    Log.d("","ggggggggget group_messages and  star :${star_msg} & message_number :${message_number}")
                } while (cursor.moveToNext())
            }
            db.close()
        } catch (e: Error) {
    ...
    fun update_group_info(operators: String, new_data: String, group_number: String) {
        ...
        if (operators == "mute_member") {
            sql_string = "UPDATE ${GROUP_INFO_TABLE} SET ${MEMBER_}=('${new_data}') WHERE ${MUTE_SPECIFIC_PERSON}='${group_number}';"
        }
    ...
    fun get_group_message_one_template(group_number: String, msg_number: String) : reaction_store_group_model? {
        var message_ : reaction_store_group_model ? = null
        val db = this.readableDatabase
        try {
        ...
        Log.d("","ddddddddddatabase of template get message data :${message_}")
        return message_
    }
