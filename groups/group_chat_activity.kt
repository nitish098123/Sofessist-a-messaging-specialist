...
import com.example.database_part_3.video_compressor.compressor
...
class group_chat_activity : AppCompatActivity() ,
    ...
    override fun onCreate(savedInstanceState: Bundle?){     // this means u can take inputs from layouts directly
        ...
        PRIVATE_GROUP = intent.getStringExtra("+private_group").toBoolean()      // if private group is on then cant screenshoot , froward , cannot copy , dont cannot save image to gallery
        ...
        PRIVATE_CHAT_ = PRIVATE_GROUP
        val lock_show = findViewById<ImageView>(R.id.lock_pair_chat_id)

        if(PRIVATE_GROUP== true){        // for restricting the screenshoot the screen
        ...
        Log.d("","ddddddddddddelta time taken to compile the above code is:${time_2-time_1}")
        ...
        val thread = Thread({
            ...
            handler.post{
                ...
                adapter = group_chat_adapter(this,this,msg_number_position,group_number)
                ...
                send_button.setOnClickListener{
                    ...
                    if(EDIT_MESSAGE == false){
                        ...
                        val send_data = group_message_model(
                                                     "${group_name}", "${group_number}", "${TOTAL_MESSAGE}", msg_data, "g_chat",
                                                     read_str, "", "none", time_, false, false, "none",
                                                     "none", MY_NUMBER, replay_status, "none"
                                                          )
                    ...
                    if (EDIT_MESSAGE == true){
                        ...
                        val DB = universal_chat_store(this, null)
                        val thread = Thread({
                            DB.update_group_message(
                                "EDIT_REWRITE",
                                group_number,
                                msg_number_,
                                msg_data
                            )   // updating only the perticular column
                        })
                        thread.start()
                        cancel_edit_text()
                        EDIT_MESSAGE = false
...
//                      initiate_message_me(all_group_chats)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data : Intent?){
        ...
        if(resultCode == Activity.RESULT_OK && requestCode == SELECT_VIDEO){
            ...
            if(data.data!=null && data.clipData==null){
                progress_bar = ProgressDialog(this)
                progress_bar.setMessage("Please wait a moment , your video is processing")
                progress_bar.show()
                ...
                val size_of_video = sizeOf("${data.data}")
                Toast.makeText(this,"Original size of video: ${size_of_video}",Toast.LENGTH_LONG).show()

                val handler = Handler()
                Thread({
                      val uri_ = data.data
                    handler.post {
                          val kk = ArrayList<Uri>()
                          kk.add(uri_!!)
                          video_compressor_(context_).compress_video(kk)
        ...
        if(resultCode == Activity.RESULT_OK && requestCode==REQUEST_CODE_ACCEPTING_DATA){
            ...
            Thread({
                for(i in all_uri){
                    ...
                    all_group_chats.add(group_message_model(group_name,group_number,"$TOTAL_MESSAGE",data_str,"g_i",
                                                            read_str,read_str,"none",get_send_time(),false,false,
                                                            "none","none", MY_NUMBER_LONG,"none","none"))

                    DB.save_group_message(group_name,group_number,"$TOTAL_MESSAGE",data_str,"g_i",
                                          read_str,read_str,"none",get_send_time(),false,false,
                                         "none","none", MY_NUMBER_LONG,"none","none")
            ...
            if(data!!.data!==null){
                ...
                val group_message_model_ = group_message_model(
                    ...
                    handler.post {     // this is connecting to the UI thread to show the messages
                        all_group_chats.add(group_message_model_)
                        initiate_message_me(all_group_chats)
...
//              var send_data : group_message_model? = null
              val handler = Handler()
              val DB = universal_chat_store(context_,null)
              Thread({
                  ...
                  for(i in recieved_data) {
                      ...
                      all_group_chats.add(send_data)
                      DB.save_group_message(group_name,
                          group_number,
                          "$TOTAL_MESSAGE",
                          single_data,
                          "contact",
        ...
        _post!!.setOnClickListener{
            ...
            if(text_.isNotEmpty()){
                if(TEMPLATE_NUMBER == "storing_reaction"){
                    ...
                    val thread : Thread = Thread({
                      db.save_group_message(group_name,group_number,"$TOTAL_MESSAGE",_message_,"storing_reaction",read_str,delivered_str,"none",
                                             _time,false,false,"yes","none",MY_NUMBER,"none","none")
                      handler.post {
...
//                sheet.cancel()
    ...
    private fun video_compressor_before_send(
        uri_: String,
        time: String,        // total duration of vedio
        size1 : String,          // original vedio link
        size2 : String,         // compressed link of the vedio
        uri_2 : String )

    {
        var final_size_container : String = ""
        val file_path : Uri = uri_.toUri()

//        val uri_2 : String = compressor().compress_video(uri_.toUri(), this).toString()    // uri of compressed vedio
...
//        val size1 = sizeOf(file_path.toString())                      // this is original size of the image
        compress_size_1.text = Html.fromHtml("ORIGINAL : <b>$size1</b>")
        uri_to_thumbnail(file_path,image_1)

//        val size2 = sizeOf(uri_2)                     // 2nd compressor size of image
        compress_size_2.text =Html.fromHtml("COMPRESSED : <b>$size2</b>")
        uri_to_thumbnail(uri_2.toUri(),image_2)

        // this are for color selections after selecting the pixels of Image
        compression_1.setOnClickListener{
            PICK_IMAGE = file_path.toString()
//          BITMAP_CONTAINER = full_size_bitmap
            color_selection_video(1)
            final_size_container = size1
        }

        compression_2.setOnClickListener{
            PICK_IMAGE = uri_2
//          BITMAP_CONTAINER = reduced_bitmap_2
            color_selection_video(2)
            final_size_container = size2
        }

        // for viewing the full image pixels
        // when clicked in image to fullView
        image_1.setOnClickListener{
            val anotherIntent = Intent(this, play_vedio_large::class.java)
            anotherIntent.putExtra("video_uri", file_path.toString())
            startActivity(anotherIntent)
            Log.d("", "vvvvvvviewing the full image: original")
        }
        ...
        image_2.setOnClickListener{
            val anotherIntent = Intent(this, play_vedio_large::class.java)
            anotherIntent.putExtra("video_uri", uri_2)
            startActivity(anotherIntent)
            Log.d("","vvvvvvviewing the full image: compressed_2")
        }
        sheet_.show()
        ...
        send_image.setOnClickListener{
            ...
            TOTAL_MESSAGE++
            val read = HashMap<String,String>()                 // contains contact number with seen time of this vedio
            val read_str = mapper.writeValueAsString(read)      // same for containing the delivered record
            val vedio_detail = video_model(time,"$final_size_container","","",PICK_IMAGE)
            val save_data = mapper.writeValueAsString(vedio_detail)                 // ready to save to data base
            val send_data = group_message_model(group_name, group_number, "$TOTAL_MESSAGE",save_data,"g_v",read_str,read_str,"none",
                                               get_send_time(),false,false,"none","none",MY_NUMBER_LONG,"none","none")
