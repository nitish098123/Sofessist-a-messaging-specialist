...
class comment_section_group : AppCompatActivity() , ActionMode.Callback{
    ...
    private lateinit var all_comments : ArrayList<group_message_model>          // all the updating message is stored here to send to the adapter
    private lateinit var thread_store : reaction_store_group_model
    ...
    private lateinit var data_ : EditText
    ...
    override fun onCreate(savedInstanceState: Bundle?){          // this means u can take inputs from layouts directly
        ...
        data_ = findViewById(R.id.text_comment_id)
            ...
            handler.post{
                ...
                val message_swipe_controller = MessageSwipeController(this,object : SwipeControllerActions {
                    override fun showReplyUI(position : Int){             // this will give the recycle View item positions number
                        ...
                        if(all_comments[position].category=="storing_reaction"){

                        }
                        if(all_comments[position].category=="voting_template"){
        ...
        send_.setOnClickListener{
            ...
            var message_ : String = data_.text.toString()
            data_.setText("")
            val read_ = HashMap<Int,String>()
            val delivered_ = HashMap<Int,String>()
            ...
            val comment_post = group_message_model(group_name,group_number,"$TOTAL_MESSAGE",message_,"g_chat",mapper.writeValueAsString(read_)
                                                  ,mapper.writeValueAsString(delivered_),"none",time_,false,false,"none","none",
                                                   MY_NUMBER,replay_status,"none")
            ...
            val DB = universal_chat_store(this,null)
            val thread = Thread({
                // this all_comments store all the messages that is sended in the thread
                DB.update_group_reaction_template("you_comment",group_number,msg_number,save_data)
                DB.close()
            })
            thread.start()
    ...
    private fun showQuotedMessage(){
        ...
        inputMethodManager.showSoftInput(data_,InputMethodManager.SHOW_IMPLICIT)
